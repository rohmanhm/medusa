...
} else if CommandLine.arguments.contains("--snapshot-keepawakepane"),
          let index = CommandLine.arguments.firstIndex(of: "--snapshot-keepawakepane"),
          CommandLine.arguments.indices.contains(index + 1) {
    delegate = SnapshotRunner(subject: .keepAwakePane, outputDir: CommandLine.arguments[index + 1])
